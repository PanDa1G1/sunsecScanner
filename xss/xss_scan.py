...
class xss_Scanner():
	...
	def judge_tag(self):
		...
		if m:
			...
			if tag:
				#print("[3]"+tag)
				self.if_tags=1
				sys.stdout.write(Fore.LIGHTYELLOW_EX +"[*]tags can be injected\n")
				return tag
			else:
				self.if_tags=1
				sys.stdout.write(Fore.LIGHTYELLOW_EX +"[*]tags can be injected\n")
	...
	def judge_attribute(self):
		...
		if m:
			...
			#print("[4]"+attribute)
			self.if_attribute=1
			sys.stdout.write(Fore.LIGHTYELLOW_EX +"[*]attribute can be injected\n")
			return attribute
		else:
			return 0
	def judge_dom(self):
		...
		result = requests.get(url,headers = self.headers).text
		#print("[7]" + result)
		if re.search("location.search",result):
			self.dom_arr = re.findall("innerHTML|document.write",result)
			self.dom_arr.append("location.search")
			self.if_dom=1
			sys.stdout.write(Fore.LIGHTYELLOW_EX +"[*]dom can be injected\n")
			sys.stdout.write(Fore.LIGHTGREEN_EX +"[*]finding: ")
			for i in self.dom_arr:
				sys.stdout.write(Fore.LIGHTGREEN_EX +"{}\t".format(i))
...
if __name__ == '__main__':
	test = xss_Scanner("http://127.0.0.1/php/2.php?a=*")
	test.run()
	#test.judge_dom()
	#test.dom_scan()
